...
**CI Podman build-container DNS (ubuntu-26.04):** the `ubuntu-26.04` runner's host resolver is the systemd-resolved stub (`127.0.0.53`), which is unreachable from inside a Podman build container's network namespace — so `apt-get`/`apk` in a `podman build` fail with `Temporary failure resolving …` (the `ubuntu-24.04` / Podman 4.9 runner did not hit this). The CI Podman job writes a `containers.conf.d` drop-in setting `dns_servers=["1.1.1.1","8.8.8.8"]` so build containers use reachable public resolvers. This is a runner/Podman-environment quirk, not a run-dockerfile concern (run-dockerfile never injects `--dns`); local Podman, which already has working resolvers, is unaffected.

**Engine caveat — Dockerfile `COPY` here-documents need Docker or Buildah ≥ 1.39:** the `COPY <<EOF … EOF` inline-file form needs a builder that parses Dockerfile here-documents. Docker/BuildKit always does; Podman only from a recent Buildah (≥ 1.39, e.g. Debian 13 trixie / Podman 5.x). An older Buildah (1.28.2, or the 1.33 bundled with Podman 4.9.3) treats `<<EOF` as a literal source path and errors `copier: stat "/<<EOF": no such file or directory`. This is a Buildah-version gap, not a permanent engine limitation. run-dockerfile passes the Dockerfile through verbatim, so a Dockerfile using `COPY <<EOF` simply needs a new-enough builder. The README's `installer-01-expect` sample uses this form; `tests/0029` builds it **unconditionally** (no engine-specific skip), and **CI runs the Podman job on `ubuntu-26.04`** (Podman 5.7 / Buildah 1.42) so its builder is new enough to match the Docker job. The README carries an engine note telling users on an older Podman/Buildah to `COPY` the script from a sibling file instead of using the inline here-document.
