...
**Distro-stripped (not an engine gap) — Dockerfile `COPY`/`RUN`/`ADD` here-documents under some Podman builds:** the `COPY <<EOF … EOF` inline-file form is a Dockerfile/BuildKit frontend feature that **upstream Buildah has supported since v1.33.0** (Nov 2023, PR #5092 — verified working here on Homebrew Podman 6.0 / Buildah 1.44, including run-dockerfile's stdin-fed `-f -` build path). It is **not** a permanent engine-capability gap: some distro packages ship Buildah with here-document support **patched out to drop a BuildKit dependency**. Debian/Ubuntu did exactly this — the Buildah behind `apt install podman` on Ubuntu 24.04 LTS (1.33.5) lacks it and fails with `copier: stat "/<<EOF": no such file or directory`; Debian restored it in `buildah 1.35.3+ds1-2` (July 2024, once Docker 26.1 was available). An earlier note here misread this as an engine gap because run-dockerfile's CI ran on the **Ubuntu-packaged** Buildah, not an upstream build. run-dockerfile passes the Dockerfile through verbatim, so whether `COPY <<EOF` builds depends on the engine's *build*, not the engine kind. `tests/0029` therefore **probes** the resolved engine with a one-step throwaway `COPY` here-document build and skips only the `installer-01-expect` sub-sample where that probe fails (message names the distro-strip cause), running it under Docker and any heredoc-capable Buildah; the README's `installer-01-expect` sample carries a matching engine note pointing affected users at a sibling-file `COPY`.
