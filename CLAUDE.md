...
- **Capability vocabulary:** `qemu-amd64`/`qemu-arm64`/`qemu-armv7` (a foreign-arch `#platform:` image runs here), `cgroups` (container cgroup limits observable), `python3` (host server for `#http.static`), `linux` (host-side Linux behavior, e.g. `/dev/pts`), `rootless-podman` (bare rootless Podman with global keep-id), `gnu-stat` (reserved, no host-side use today). Only gated tests carry a tag: `0002 0003 0004` (qemu-*), `0008 0029 0030` (python3), `0010` (linux), `0011 0014` (cgroups), `0046` (rootless-podman); the rest are engine/OS-agnostic and run in every cell.
- **5-cell CI:** a reusable workflow `.github/workflows/cell.yml` (`workflow_call`, inputs `runner-json`/`engine`/`userns`/`cell-caps`/`setup-*`) runs `tests/run --all` once per cell; five thin caller workflows give one badge each — `linux-docker`, `linux-podman-rootful`, `linux-podman-rootless` (bare `podman` + `RUN_DOCKERFILE_USERNS=keep-id`), `macos-docker`, `macos-podman` (Podman in a VM, `cell-caps` started conservatively). The old single `test.yml` with its hand-maintained `grep -vE '^000[234]_|^0046_'` exclusion list is gone; cell membership is now derived from caps.
