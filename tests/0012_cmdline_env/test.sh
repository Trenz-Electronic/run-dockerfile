...
# Test -e VAR=val
output=$(./run -e TEST_A=val_a sh -c 'echo "TEST_A=$TEST_A"') || true
case "$output" in
...
# Test --env VAR=val
output=$(./run --env TEST_B=val_b sh -c 'echo "TEST_B=$TEST_B"') || true
case "$output" in
...
# Test --env=VAR=val
output=$(./run --env=TEST_C=val_c sh -c 'echo "TEST_C=$TEST_C"') || true
case "$output" in
...
output=$(./run -e TEST_D=val_d -e TEST_E=val_e sh -c 'echo "D=$TEST_D E=$TEST_E"') || true
case "$output" in
